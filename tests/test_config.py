...
class TestLLMConfig:
    ...
    def test_env_override(self) -> None:
        """Environment variables prefixed with HELIX_LLM_ override defaults."""
        with patch.dict(
            os.environ,
            {
                'HELIX_LLM_API_KEY': 'env-key',
                'HELIX_LLM_CHAT_MODEL': 'gpt-4-turbo',
                'HELIX_LLM_MAX_TOKENS': '8192',
                'HELIX_LLM_TEMPERATURE': '0.3',
            },
        ):
            cfg = LLMConfig()
...
class TestSettings:
    ...
    def test_nested_env_override(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                'HELIX_LLM_API_KEY': 'top-level-key',
